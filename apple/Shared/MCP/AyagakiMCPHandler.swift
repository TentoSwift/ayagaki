...
final class AyagakiMCPHandler: @unchecked Sendable {
    ...
    private func dispatch(method: String, params: [String: Any]) async throws -> [String: Any] {
        ...
        case "initialize":
            ...
            return [
                ...
                get_notation で組むときの交換記号を取得できます（上n=n目入れかえ・下n=n目そのまま・丸数字=元に戻る・ナミ=入れかえなし、末尾の下は省略）。
    ...
    static let toolDefinitions: [[String: Any]] = [
        ...
        [
            "name": "get_notation",
            "description": "デザインの交換記号（組むときの手順）を取得する。段ごとに左右半面の記号を返し、同じ手順が続く段はまとめられる。上n=n目入れかえて浮かせる、下n=n目そのまま（末尾は省略）、丸数字=入れかえた糸が元に戻る、ナミ=入れかえなし。",
            "inputSchema": [
                "type": "object",
