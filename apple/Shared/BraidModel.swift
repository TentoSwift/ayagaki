    ...
    static func sideSymbols(_ plane: [[Int]], arrows: [Bool]? = nil,
        ...
        for (r, row) in plane.enumerated() {
            ...
            func scheduleReturn(_ end: (row: Int, col: Int, steps: Int)) {
                ...
                // 着地が一番端の列で、その端の対が着地の行範囲 [end.row, end.row+step] で
                // 糸交換を発火しているなら、その対は別の交換で使用中のため戻せない
                // （行範囲は配列境界でクランプ）
                if rCol == cap - 1 {
                    let rLo = max(0, end.row)
                    let rHi = min(rows - 1, end.row + step)
                    for rr in rLo...rHi where fired[rr][rCol] { return }
                }
