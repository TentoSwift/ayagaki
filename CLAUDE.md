...
- 記号 = [糸交換の数字] + [綾名] + [⬆]：
  ...
  - **縦に進む列（偶数列 d=2,4,…）は糸交換のみ**（綾名には入れない。ユーザー確認 2026-07-07）。番号は中央=1 からの通し（d+1: 3,5,7…13）。色のある段にその番号を書き、+2の丸数字で戻す。**戻しは色が続いている間はできず、その目の色が終わった段の2段先**（交換した段の2段先より先になることがある。ユーザー確認 2026-07-07）。**糸交換・戻しの番号は片面の目数（60玉=13）で頭打ち**
