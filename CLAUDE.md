...
- 記号 = [糸交換の数字] + [綾名] + [⬆]：
  ...
  - **縦に進む列（偶数列 d=2,4,…）は糸交換のみ**（綾名には入れない。ユーザー確認 2026-07-07）。番号は中央=1 からの通し（d+1: 3,5,7…13）。色のある段にその番号を書き、2段先に+2の丸数字で戻す（例: d=2 を塗った段「3.ナミ」→ 2段先「⑤ナミ」）。**糸交換・戻しの番号は片面の目数（60玉=13）で頭打ち**
