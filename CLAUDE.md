...
- 記号 = [糸交換の数字] + [綾名] + [⬆]：
  ...
  - **斜めに進んだ縞は一番先で糸交換**（ユーザー確認 2026-07-07）: 入れかえの目を内側から外へ移ってきた色の縞は、一番先の目の色が終わる段に、その場所の番号（d+1）を1回だけ書く。戻しは不要。判定＝内側（d-2、d=1は中央の上ル）から続いてきて、外側（d+2）へは続かず、終端の段で内側の色が消えている run
