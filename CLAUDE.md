...
- 記号 = [糸交換の数字] + [綾名] + [⬆]：
  ...
  - **縦に進む列（偶数列 d=2,4,…）は糸交換のみ**（綾名には入れない。ユーザー確認 2026-07-07）。番号は中央=1 からの通し（d+1: 3,5,7…13）。色のある段にその番号を書く。**戻しは、交換した対が45度（1段ごとに1目外）で進み、色が地色になる所**＝45°に色を辿った最後の位置の2段先の段に、その位置の番号+3の丸数字（色が1段だけなら交換の2段先に+2。交換の2段先より先になることがある。ユーザー確認 2026-07-07）。**糸交換・戻しの番号は片面の目数（60玉=13）で頭打ち**
