...
# (塗り, ⬆の段(0始まり・左半面), 期待値)。⬆は書籍 4-15 の左列に忠実
FIXTURES = {
  "4-15 ブロック1": ([[],[1],[1],[]], [0,1],
    ["ナミ⬆","上1⬆","上1","②③④ナミ"]),
  ...
  # ⬆を伴わない模様の戻しは入れかわっていた目の糸番号のみ（ユーザー確認 2026-07-06）
  "⬆なし 上1×1段": ([[],[1],[],[]], [],
    ["ナミ","上1","②ナミ","ナミ"]),
  "⬆なし 上2×2段": ([[],[1,2],[1,2],[]], [],
    ["ナミ","上2","上2","②③ナミ"]),
...
failed = 0
try:
    for name, (slices, arrowRows, expected) in FIXTURES.items():
        rows = len(slices)
        ...
        res = mcp([("update_design", {"id":ID,"rows":rows}),
                   ("set_cells", {"id":ID,"cells":{"L":grid,"R":grid,"C":carr}}),
                   ("get_notation", {"id":ID})])
        got = [None]*rows
